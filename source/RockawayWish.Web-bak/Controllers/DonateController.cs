...
namespace RockawayWish.Web.Controllers
{
    ...
    {
        private AccountsProvider _userProvider = new AccountsProvider();
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                bool isDonator = true;
                // register user
                var result = await _userProvider.Create(new Guid(SiteConfig.ApplicationId), model.Email, Guid.NewGuid().ToString(), model.FirstName, model.LastName, true, false, isDonator, false, false, model.YearJoined, model.Address, model.City, model.State, model.Country, model.Zip, model.Phone, model.CellPhone);

                if (result.Status == 1)
                    Session["DonationUserId"] = new AccountsProvider().Get(new Guid(SiteConfig.ApplicationId)).Result.Where(x => x.Email.Equals(model.Email)).FirstOrDefault().UserId;
        ...
        {
            var user = new AccountsProvider().GetById(this.ApplicationId, new Guid(Session["DonationUserId"].ToString())).Result;
